...
// 0x514910771AF9Ca656af840dff83E8264EcF986CA
    function trade(address _tradeToken, uint256 _tradeAmount) public payable {
          ...
          // must approve pooltoken to the contract
            ...
            uint256 previousTradeTokenBalance = tradedToken.balanceOf(address(this));
            swapTokenForToken(address(poolToken),address(_tradeToken),3000,_tradeAmount,address(this));
           
            tradeTokens++;
               uint256 newTradeTokenBalance = tradedToken.balanceOf(address(this));
            uint256 tradeTokenAmount = newTradeTokenBalance-previousTradeTokenBalance;
            ...
            for(uint256 i=1;i<=membersOfPools;i++){
                ...
                for(uint256 i=1;i<=membersOfPools;i++){
                    uint256 userShare = memberTokenBalance[memberAddress[i]]*1e18/poolTokenAmount;
                    uint256 userTradeAmount = (_tradeAmount*userShare)/1e18;
                    memberTokenBalance[memberAddress[i]] -=userTradeAmount;
     ...
     function dca(uint256 tradeTokenId, uint256 _tradeAmount) public payable {
          ...
          // must approve pooltoken to the contract
            ...
            swapTokenForToken(address(poolToken),address(_tradeToken),3000,_tradeAmount,address(this));
                  uint256 newTradeTokenBalance = tradedToken.balanceOf(address(this));
           uint256 newTradeAmount = newTradeTokenBalance-previousTradeTokenBalance;
                  
        //    uint256 newTradeAmount = tradedToken.balanceOf(address(this))-tradeTokenById[tradeTokenId].tokenTotalBalance;
            uint256 tradeTokenAmount = newTradeAmount+tradeTokenById[tradeTokenId].tokenTotalBalance;
         ...
         function sellToken(uint256 tradeTokenId) public payable {
             IERC20 poolTokenBalance = IERC20(poolToken);
                ...
                address tradedToken = tradeTokenById[tradeTokenId].tokenAddress;
                require(tradedToken!=address(0),"No pool found");
                uint256 tokenAmount = tradeTokenById[tradeTokenId].tokenTotalBalance;
                require(tokenAmount>0,"No token in pool");

                swapTokenForToken(tradedToken,address(poolToken),3000,tokenAmount,address(this));

                uint256 newBalance = poolTokenBalance.balanceOf(address(this));
                uint256 poolBalance= newBalance-previousBalance;

                for(uint256 i=1;i<=membersOfPools;i++){
                    uint256 userShare = tradeTokenBalance[memberAddress[i]][tradeTokenId]*1e18/tokenAmount;
                    uint256 userTradeAmount = (poolBalance*userShare)/1e18;
                    memberTokenBalance[memberAddress[i]] +=userTradeAmount;

                    uint256 userTokenAmount = (tokenAmount*userShare)/1e18;
                    tradeTokenBalance[memberAddress[i]][tradeTokens] -=userTokenAmount;
                    
                }
                 poolTokenAmount+=poolBalance;


            uint256 tokenExitPrice = tokenAmount*1e18/poolBalance;
            tradeTokenById[tradeTokenId].tokenTotalBalance -=tokenAmount; 
            tradeTokenById[tradeTokenId].tokenExitPrice =tokenExitPrice ;
            tradeTokenById[tradeTokenId].tradeStatus = Status.Finished;
...
contract PoolManager{
    ...
    function getAllPoolsStruct() public view returns(
       poolObj[] memory
        ) {
        
            poolObj[] memory totalPools = new poolObj[](pools);

           
           
            for(uint256 i = 1;i<=pools;i++){
                Pool pool = Pool(payable(poolbyId[i]));
                totalPools[i-1].poolAddress = poolbyId[i];
                 totalPools[i-1].poolOwner = pool.poolAdmin();
    ...
    // Pool Admin, Pool Token, Pool TVL, Pool Trades
